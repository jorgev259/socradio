...
export default function Player (props) {
  ...
  useEffect(() => {
    if (audio) {
      audio.src = `https://radio.sittingonclouds.net/play/${props.station}`
      audio.load()
      audio.play()
    }
    initiateSocket()
  ...
  return (
    <Container className={classnames(styles.player, 'text-center px-2 px-md-0 h-100 d-flex align-items-center justify-content-center flex-column')}>
      <audio ref={audioRef} preload='none' id='erplayer-audio' src={`https://radio.sittingonclouds.net/play/${props.station}`} volume={volume} />
